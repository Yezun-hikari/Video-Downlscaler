...
def compress_video(input_path, output_path, target_size_mb, progress_callback=None):
    ...
    # Safety margin: Target 95% of the requested size to account for container overhead and muxing.
    effective_target_size = target_size_mb * 0.95

    target_total_bitrate = (effective_target_size * 8 * 1024 * 1024) / duration

    # Audio bitrate - Keep high quality (minimum 192k) to fix "dull" audio issues.
